...
class CharTfidfTagger(BaseEstimator):
    ...
    def set_params(self, **params):
        self.tfidf.set_params(**{k.split("__", 1)[-1]: v for k, v in params.items() if k.startswith("tfidf__")})
        self.tagger.set_params(**{k.split("__", 1)[-1]: v for k, v in params.items() if k.startswith("tagger__")})
    
    def get_params(self, deep=True):
        params = {'tfidf__' + k: v for k, v in self.tfidf.get_params(deep).items()}
        params.update({'tagger__' + k: v for k, v in self.tagger.get_params(deep).items()})
        return params
    ...
    def fit(self, X, y):
        ...
        features = [
            self.featurize(example) for example in X
        ]
    ...
    def featurize(self, tokens):
        return [dict(zip(self.tfidf.get_feature_names(), self.tfidf.transform([token]).toarray().reshape(-1))) for token in tokens]