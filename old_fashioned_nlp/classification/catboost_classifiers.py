...
class TfidfCatBooostClassifier(BaseEstimator):
    ...
    def set_params(self, **params):
        ...
        self.model.set_params(**{k.split("__", 1)[-1]: v for k, v in params.items() if k.startswith("classifier__")})
    
    def get_params(self, deep=True):
        params = {'tfidf__' + k: v for k, v in self.tfidf.get_params(deep).items()}
        params.update({'classifier__' + k: v for k, v in self.model.get_params(deep).items()})
        return params
